...
import { Plus, Pencil, UserX, UserCheck, X, Camera, ShieldCheck, User, Trash2, Eye, EyeOff } from 'lucide-react';
...
export default function UsersPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Usuario creado'); queryClient.invalidateQueries(['users']); closeModal(); },
  ...
  const deactivateMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Usuario desactivado'); queryClient.invalidateQueries(['users']); },
    onError: (err) => toast.error(err?.response?.data?.error || 'Error'),
  });

  const activateMutation = useMutation({
    mutationFn: activateUser,
    onSuccess: () => { toast.success('Usuario activado'); queryClient.invalidateQueries(['users']); },
    onError: () => toast.error('Error'),
  });

  const deleteMutation = useMutation({
    mutationFn: permanentDeleteUser,
    onSuccess: () => { toast.success('Usuario eliminado'); queryClient.invalidateQueries(['users']); },
  ...
  const openEdit = (user) => {
    setForm({ name: user.name, email: user.email, role: user.role, password: '', currentPassword: '', newPassword: '' });
    setShowPass(EMPTY_SHOW);
    setPhotoFile(null);
    ...
    <motion.div variants={fadeIn} initial="hidden" animate="visible">
      <motion.div variants={fadeInUp} initial="hidden" animate="visible"
        ...
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">{data?.data?.length ?? 0} usuarios registrados</p>
        </div>
        <motion.button whileHover={buttonHover} whileTap={buttonTap} onClick={openCreate}
              ...
              <motion.tbody variants={staggerContainer} initial="hidden" animate="visible"
                ...
                {data?.data?.map((u) => (
                  <motion.tr key={u.id} variants={fadeInUp}
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <motion.button onClick={() => openEdit(u)}
          ...
          >
            ...
            >
              <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-[#2e3650]">
                ...
                <button onClick={closeModal} className="p-1.5 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 rounded-lg hover:bg-gray-100 dark:hover:bg-[#2e3650] transition-colors">
                  <X size={18} />
                </button>
              </div>
                  ...
                  <details className="group">
                    ...
                    <div className="mt-3 space-y-3">
                      {modal.user.id === currentUser?.id && (
                        <div>
                          <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Contraseña actual</label>
                          <PasswordInput
                      ...
                      <div>
                        <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Nueva contraseña</label>
                        <PasswordInput
