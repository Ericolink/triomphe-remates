...
export default function AuditPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => getAuditLogs({ action: action || undefined, resource: resource || undefined, page, limit: 30 }),
  });

  const select = 'px-3 py-2 border border-gray-200 dark:border-[#2e3650] rounded-xl text-sm bg-white dark:bg-[#242938] dark:text-gray-100 focus:outline-none';

  return (
    <motion.div variants={fadeIn} initial="hidden" animate="visible">
      <motion.div variants={fadeInUp} initial="hidden" animate="visible" className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2">
            <ShieldCheck size={22} className="text-blue-700 dark:text-blue-400" /> Audit Log
          </h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">{data?.pagination?.total ?? 0} eventos registrados</p>
        </div>
        <div className="flex gap-2">
          <select value={action} onChange={(e) => { setAction(e.target.value); setPage(1); }} className={select}>
            {ACTIONS.map((a) => <option key={a} value={a}>{a ? actionLabel[a] : 'Todas las acciones'}</option>)}
          </select>
          <select value={resource} onChange={(e) => { setResource(e.target.value); setPage(1); }} className={select}>
            {RESOURCES.map((r) => <option key={r} value={r}>{r ? resourceLabel[r] : 'Todos los recursos'}</option>)}
          </select>
        </div>
      </motion.div>
      ...
      {!isLoading && data?.data?.length === 0 && (
        <div className="text-center py-16 text-gray-400 dark:text-gray-500">No hay eventos con este filtro</div>
      )}
