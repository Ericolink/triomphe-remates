...
import { Trash2, MessageSquare, AlertCircle, Lightbulb, FileSpreadsheet, Archive, Search } from 'lucide-react';
...
export default function BuzonAdminPage() {
  ...
  const items = search.trim()
    ? allItems.filter((i) => {
        ...
        return i.name?.toLowerCase().includes(q) || i.subject?.toLowerCase().includes(q) || i.message?.toLowerCase().includes(q);
      })
    : allItems;
  ...
  const toggleAll = () => { const ids = items.map((i) => i.id); setChecked(checked.length === ids.length ? [] : ids); };
  const handleArchive = (e, item) => { e.stopPropagation(); updateMutation.mutate({ id: item.id, data: { status: 'archivado' } }); };
  const handleQuickDelete = (e, item) => {
    e.stopPropagation();
    setConfirm({
      title: '¿Eliminar este mensaje?',
      message: `Se eliminará el mensaje de ${item.name} permanentemente.`,
      onConfirm: () => { deleteMutation.mutate(item.id); setConfirm(null); },
    });
  };
      ...
      <motion.div variants={fadeInUp} initial="hidden" animate="visible" className="mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          <div className="flex items-center gap-2 bg-white dark:bg-[#242938] border border-gray-200 dark:border-[#2e3650] rounded-xl px-3 py-2 flex-1 min-w-[220px]">
            ...
            <input type="text" placeholder="Buscar por nombre, asunto o mensaje..." value={search}
        ...
        <div className="lg:col-span-2 space-y-3">
                ...
                {items.map((item) => {
                  ...
                  <motion.div key={item.id}
                        ...
                        <div className="flex items-start justify-between mb-2">
                          <div className="flex-1 min-w-0 mr-3">
                            ...
                            <p className="text-xs text-gray-400 dark:text-gray-500">
                              ...
                              {isArchived && <span className="ml-1 italic">· {FEEDBACK_STATUS_LABELS.archivado}</span>}
                            </p>
                          </div>
                          <Badge variant={FEEDBACK_CATEGORY_VARIANTS[item.category]}>
                            <span className="flex items-center gap-1">{categoryIcon[item.category]} {FEEDBACK_CATEGORY_LABELS[item.category]}</span>
                          </Badge>
                        </div>
                        <p className="text-xs text-gray-400 dark:text-gray-500 line-clamp-2 pr-12">{item.message}</p>
                      </div>
                    </div>
                    <div className="absolute bottom-3 right-3 z-10 flex items-center gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                      {!isArchived && (
                        <button onClick={(e) => handleArchive(e, item)} title="Archivar" aria-label={`Archivar mensaje de ${item.name}`}
                      ...
                      <button onClick={(e) => handleQuickDelete(e, item)} title="Eliminar" aria-label={`Eliminar mensaje de ${item.name}`}
          ...
          {!isLoading && items.length === 0 && (
            <motion.div variants={fadeIn} initial="hidden" animate="visible"
              ...
              {search.trim() ? 'Ningún mensaje coincide con la búsqueda' : 'No hay mensajes con este filtro'}
            </motion.div>
          )}
        </div>
                ...
                <div className="space-y-3 mb-6 text-sm">
                  ...
                  <div>
                    ...
                    <Badge variant={FEEDBACK_CATEGORY_VARIANTS[selected.category]}>
                      <span className="flex items-center gap-1">{categoryIcon[selected.category]} {FEEDBACK_CATEGORY_LABELS[selected.category]}</span>
                    </Badge>
                  </div>
                  <div>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <select value={selected.status}
                      ...
                      {Object.entries(FEEDBACK_STATUS_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
                    </select>
                  </div>
                  <div>
