...
const processSteps = [
  ...
  { step: 2,  icon: CalendarDays,  title: 'Agendar una cita',          desc: 'Se coordina la primera reunión con el cliente interesado para presentar la propiedad con plano catastral y adeudos.' },
  { step: 3,  icon: Info,          title: 'Información de la propiedad',desc: 'Se proporciona al cliente toda la información necesaria: dictamen legal, plano catastral, adeudos de agua y predial, y cualquier detalle monetario relevante.' },
  { step: 4,  icon: Search,        title: 'Verificación legal',         desc: 'Se revisa la disponibilidad y viabilidad legal de la propiedad. Se confirman resultados positivos antes de continuar.' },
  { step: 5,  icon: Tag,           title: 'Apartado de la propiedad',   desc: 'El cliente realiza el apartado de la propiedad con el monto requerido, asegurando su reserva en el inventario.' },
  { step: 6,  icon: FileSignature, title: 'Firma de contrato',          desc: 'Se formaliza la operación mediante la firma de los contratos correspondientes con todos los detalles acordados.' },
  { step: 7,  icon: Banknote,      title: 'Pago del remate',            desc: 'El cliente realiza el pago o transferencia a la institución financiera (banco) correspondiente y a Triomphe.' },
  { step: 8,  icon: PenLine,       title: 'Firma de cesión de derechos',desc: 'El cliente firma su cesión de derechos ante notario público. Documento inscrito en el Registro Público de la Propiedad.' },
  { step: 9,  icon: Scale,         title: 'Periodo legal',              desc: 'Se gestiona el seguimiento de las etapas legales. El asesor mantiene comunicación mensual con el cliente vía WhatsApp.' },
  { step: 10, icon: ScrollText,    title: 'Escrituración',              desc: 'Se completan todos los trámites notariales y legales necesarios para la escrituración de la propiedad.' },
  { step: 11, icon: Home,          title: 'Entrega de propiedad',       desc: 'El cliente recibe su propiedad. Culminación exitosa del proceso de adquisición del remate bancario.' },
];

const values = [
  'Trabajo en equipo', 'Respeto', 'Innovación', 'Orientación al cliente',
  'Motivación', 'Principios', 'Honradez', 'Entusiasmo',
];
      ...
      <section className="bg-gray-50 dark:bg-[#242938] py-16">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-8"
            variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            {[
              { icon: <TrendingDown size={32} className="text-yellow-500" />, title: 'Precios de Remate', desc: 'Propiedades hasta un 40% más baratas que el valor comercial. La mejor inversión del mercado con garantías hipotecarias.' },
              { icon: <Shield size={32} className="text-yellow-500" />, title: 'Proceso Seguro', desc: 'Cesiones de derechos inscritas en el RPP, respaldadas por instituciones financieras. Acompañamiento en todo el proceso legal y notarial.' },
              { icon: <Users size={32} className="text-yellow-500" />, title: 'Asesoría Personalizada', desc: 'Nuestros coordinadores y asesores te guían en cada etapa, desde el primer contacto hasta la entrega de tu propiedad.' },
            ].map(({ icon, title, desc }) => (
        ...
        <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-8"
          ...
          <motion.div variants={fadeInLeft}
            ...
            <div className="bg-blue-900 dark:bg-[#0f1621] text-white rounded-2xl p-8 h-full">
              <motion.div className="w-12 h-12 bg-yellow-400 rounded-xl flex items-center justify-center mb-4"
          ...
          <motion.div variants={fadeInRight}
            ...
            <div className="bg-yellow-400 dark:bg-yellow-500 rounded-2xl p-8 h-full">
              <motion.div className="w-12 h-12 bg-blue-900 rounded-xl flex items-center justify-center mb-4"
                whileHover={{ scale: 1.1, rotate: 5 }} transition={{ duration: 0.2 }}>
        ...
        <div className="relative mt-8">
          ...
          <motion.div className="space-y-6"
            ...
            {processSteps.map(({ step, icon: Icon, title, desc }) => (
              <motion.div key={step} variants={fadeInUp} className="flex gap-6 items-start">
