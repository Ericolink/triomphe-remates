...
const notAvailable = () => (
  <span className="text-gray-300 dark:text-gray-600 italic">No disponible</span>
);
      ...
      <div className="max-w-2xl mx-auto px-4 py-24 text-center">
        ...
        <motion.div variants={fadeInUp} initial="hidden" animate="visible">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-8">
            Agrega al menos 2 propiedades para compararlas.
          </p>
          <Link
            to="/propiedades"
            ...
            <tr>
              ...
              {items.map((p) => (
                <th key={p.id} className="px-2 sm:px-3 pb-4 align-top">
                  <div
                    className={`bg-white dark:bg-[#242938] rounded-2xl overflow-hidden border shadow-sm ${p.unavailable ? 'border-dashed border-gray-200 dark:border-[#2e3650] opacity-60' : 'border-gray-100 dark:border-[#2e3650]'}`}
                  >
                    <div className="relative h-24 sm:h-32 md:h-40 bg-gray-100 dark:bg-[#2e3650]">
                      {p.unavailable ? (
                        ...
                        <img
                          src={buildImageUrl(p.images[0].url, 300)}
                          alt={p.title}
                          loading="lazy"
                          decoding="async"
                    ...
                    <div className="p-2 sm:p-3">
                      {p.unavailable ? (
                        <p className="font-semibold text-xs sm:text-sm text-gray-400 dark:text-gray-500 line-clamp-2">
                          {p.title}
                        </p>
                      ) : (
                        <Link
          ...
          <tbody>
            {ROWS.map(({ label, render }) => (
              <tr key={label} className="border-t border-gray-100 dark:border-[#2e3650]">
                <td className="py-3 pr-2 sm:pr-4 text-xs font-medium text-gray-500 dark:text-gray-400 whitespace-nowrap">
                  {label}
                </td>
                {items.map((p) => (
                  <td
                    key={p.id}
                    className="px-2 sm:px-3 py-3 text-xs sm:text-sm text-gray-700 dark:text-gray-200 text-center"
                  >
