...
import { Briefcase, MapPin, Clock, ChevronDown, ChevronUp, Send, Users, Star, TrendingUp, DollarSign } from 'lucide-react';
...
function ApplicationForm({ positionId, positionTitle, onClose }) {
  ...
  const inputClass = "w-full px-3 py-2.5 border border-gray-200 dark:border-[#2e3650] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-[#1a1f2e] dark:text-gray-100 dark:placeholder-gray-500";
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        ...
        <div className="p-6 border-b border-gray-100 dark:border-[#2e3650]">
          ...
          {positionTitle && <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{positionTitle}</p>}
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" placeholder="614 000 0000" value={form.phone}
            ...
            <div>
              <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Experiencia en ventas *</label>
            ...
            <div className="flex items-center gap-3 pt-4">
              ...
              <label htmlFor="hasVehicle" className="text-sm text-gray-700 dark:text-gray-300">Cuento con vehículo propio</label>
            </div>
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">¿Por qué quieres unirte a Triomphe?</label>
            <textarea placeholder="Cuéntanos tu motivación..." value={form.motivation}
...
function PositionCard({ position }) {
  ...
  return (
    <>
      <motion.div variants={fadeInUp}
        ...
        <div className="p-6">
          <div className="flex items-start justify-between gap-4 mb-3">
            ...
            <motion.button onClick={() => setApplying(true)}
              whileHover={buttonHover} whileTap={buttonTap}
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed line-clamp-2">{position.description}</p>

          <button onClick={() => setExpanded(!expanded)}
            className="flex items-center gap-1 text-blue-600 dark:text-blue-400 text-sm font-medium mt-3 hover:underline">
            {expanded ? <><ChevronUp size={16} /> Ver menos</> : <><ChevronDown size={16} /> Ver más detalles</>}
          </button>
        </div>
          ...
          {expanded && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h4 className="font-semibold text-blue-900 dark:text-white text-sm mb-2">Descripción completa</h4>
                  <p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed">{position.description}</p>
                </div>
                <div>
                  <h4 className="font-semibold text-blue-900 dark:text-white text-sm mb-2">Requisitos</h4>
                  <p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed whitespace-pre-line">{position.requirements}</p>
                </div>
                {position.benefits && (
                  <div className="md:col-span-2">
                    <h4 className="font-semibold text-blue-900 dark:text-white text-sm mb-2">Beneficios</h4>
                    <p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed whitespace-pre-line">{position.benefits}</p>
                  </div>
                )}
              </div>
            </motion.div>
      ...
      <AnimatePresence>
        {applying && (
          <ApplicationForm positionId={position.id} positionTitle={position.title} onClose={() => setApplying(false)} />
        )}
      </AnimatePresence>
    </>
  );
    ...
    <div>
      ...
      <section className="bg-gradient-to-br from-blue-900 to-blue-700 dark:from-[#0f1621] dark:to-[#1a1f2e] text-white py-20">
        <motion.div className="max-w-7xl mx-auto px-4 text-center"
          ...
          <motion.p variants={fadeInUp} className="text-blue-200 dark:text-gray-400 text-lg max-w-2xl mx-auto mb-8">
            Sé parte de Triomphe Bienes Raíces y construye una carrera exitosa en el sector inmobiliario.
            Comisiones atractivas y crecimiento profesional.
          </motion.p>
          <motion.button variants={fadeInUp} onClick={() => setGeneralApplying(true)}
            whileHover={buttonHover} whileTap={buttonTap}
      ...
      <section className="bg-gray-50 dark:bg-[#242938] py-16">
        <div className="max-w-7xl mx-auto px-4">
          <AnimatedSection>
            <h2 className="text-3xl font-bold text-blue-900 dark:text-white text-center mb-12">¿Por qué unirte a Triomphe?</h2>
          </AnimatedSection>
      ...
      <section className="max-w-7xl mx-auto px-4 py-16">
        <AnimatedSection className="mb-8">
          <h2 className="text-3xl font-bold text-blue-900 dark:text-white mb-2">Vacantes disponibles</h2>
          <p className="text-gray-500 dark:text-gray-400">
            {positions.length > 0 ? `${positions.length} vacante(s) activa(s)` : 'Explora nuestras oportunidades'}
          </p>
        </AnimatedSection>
        ...
        {isLoading ? (
          ...
          <motion.div variants={fadeInUp} initial="hidden" animate="visible"
            ...
            <p className="text-xl font-medium text-gray-500 dark:text-gray-400">No hay vacantes activas por el momento</p>
            <p className="text-sm text-gray-400 dark:text-gray-500 mt-2 mb-6">Pero puedes enviarnos tu postulación general</p>
            <motion.button onClick={() => setGeneralApplying(true)}
              whileHover={buttonHover} whileTap={buttonTap}
      ...
      <AnimatePresence>
        {generalApplying && (
          <ApplicationForm positionId="general" positionTitle={null} onClose={() => setGeneralApplying(false)} />
        )}
      </AnimatePresence>
    </div>
  );
