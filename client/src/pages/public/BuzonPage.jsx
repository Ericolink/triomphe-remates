...
const categories = [
  ...
  { value: 'comentario', label: 'Comentario', description: 'Quiero compartir algo', color: 'border-blue-300 text-blue-700 bg-blue-50 dark:bg-blue-900/20 dark:text-blue-400 dark:border-blue-700' },
  { value: 'sugerencia', label: 'Sugerencia', description: 'Tengo una idea', color: 'border-green-300 text-green-700 bg-green-50 dark:bg-green-900/20 dark:text-green-400 dark:border-green-700' },
];
  ...
  if (sent) {
    return (
      <div className="max-w-xl mx-auto px-4 py-24 text-center">
        <SEO title="Buzón de opiniones" description="Comparte tu queja, comentario o sugerencia con Triomphe." url="/buzon" />
        <motion.div variants={fadeInUp} initial="hidden" animate="visible">
          <div className="w-16 h-16 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center mx-auto mb-6">
            <CheckCircle size={36} className="text-green-600 dark:text-green-400" />
          </div>
          <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-3">¡Gracias por tu mensaje!</h1>
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-16">
      ...
      <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="text-center mb-10">
        <motion.div variants={fadeInUp} className="w-14 h-14 bg-blue-900 rounded-2xl flex items-center justify-center mx-auto mb-4">
          <MessageSquare size={28} className="text-yellow-400" />
        </motion.div>
        <motion.h1 variants={fadeInUp} className="text-3xl font-bold text-blue-900 dark:text-white mb-2">
          Buzón de opiniones
        </motion.h1>
      ...
      <motion.div variants={fadeInUp} initial="hidden" animate="visible"
          ...
          {categories.map(({ value, label, description, color }) => (
            <button
              ...
              className={`p-3 rounded-xl border-2 text-left transition-all ${
                form.category === value ? color : 'border-gray-200 dark:border-[#2e3650] text-gray-500 dark:text-gray-400 hover:border-gray-300 dark:hover:border-[#3a4060]'
              }`}
            >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Asunto *</label>
            <input
              name="subject"
              value={form.subject}
              onChange={handleChange}
          ...
          <div>
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Mensaje *</label>
            <textarea
              name="message"
              value={form.message}
              onChange={handleChange}
            ...
            />
            <p className="text-xs text-gray-400 dark:text-gray-500 text-right mt-1">{form.message.length}/2000</p>
          </div>
