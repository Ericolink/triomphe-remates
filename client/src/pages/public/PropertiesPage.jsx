...
export default function PropertiesPage() {
  ...
  // Deriva filtros: searchParams tiene prioridad (vienen del home),
  // localFilters son los que el usuario ajusta en esta página
  const filters = {
  ...
  const setFilter = (key, value) => {
    ...
    setPage(1);
    // Limpiar el searchParam correspondiente si el usuario cambia el filtro manualmente
                ...
                <button key={p} onClick={() => setPage(p)}
                  className={`w-10 h-10 rounded-lg text-sm font-medium transition-colors ${
                    page === p
                      ...
                      : 'bg-gray-100 dark:bg-[#242938] text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-[#2e3650]'
                  }`}>
                  {p}
                </button>
              ))}
