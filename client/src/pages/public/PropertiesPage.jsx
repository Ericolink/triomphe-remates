  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10">
      <SEO
        ...
        description="Explora nuestro inventario de remates bancarios en Chihuahua, Ciudad Juárez y Querétaro."
        url="/propiedades"
      />
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <input
          ...
          className="flex-1 px-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-800 dark:text-white"
        />
        <button onClick={() => setShowFilters(!showFilters)}
          className="flex items-center gap-2 px-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors dark:text-gray-200">
      ...
      {showFilters && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl">
          ...
          <div>
            ...
            />
            {filters.maxPrice && (
              <p className="text-xs text-blue-600 mt-1">$ {Number(filters.maxPrice).toLocaleString('es-MX')} MXN</p>
            )}
          </div>
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 text-gray-400">
          ...
          <button onClick={clearFilters} className="mt-4 text-blue-600 hover:underline text-sm">Limpiar filtros</button>
        </div>
      ) : (
        <>
              ...
              {Array.from({ length: data.pagination.totalPages }, (_, i) => i + 1).map((p) => (
                <button key={p} onClick={() => setFilters((f) => ({ ...f, page: p }))}
                  className={`w-10 h-10 rounded-lg text-sm font-medium transition-colors ${
                    filters.page === p ? 'bg-blue-900 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'
                  }`}>
                  {p}
                </button>
              ))}
