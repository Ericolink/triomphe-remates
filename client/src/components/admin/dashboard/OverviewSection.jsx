  ...
  const kpis = [
    {
      ...
      value: `${ventasSemana?.count ?? 0} · ${formatPrice(ventasSemana?.total)}`,
    ...
    <div className="mb-6">
      ...
      >
        {kpis.map(({ label, value, icon, color, onClick }) => (
          ...
          >
            <div
              className={`w-10 h-10 ${color} rounded-xl flex items-center justify-center text-white mb-3`}
            >
              {icon}
            </div>
        ...
        >
          ...
          {(mejoresCampanas ?? []).length === 0 ? (
            <p className="text-sm text-gray-400 dark:text-gray-500">
              Todavía no hay prospectos asociados a una campaña.
            </p>
          ) : (
            <div className="space-y-2">
              {mejoresCampanas.slice(0, 3).map((c) => (
                <div
                  key={c.campaignId}
                  className="flex items-center justify-between px-3 py-2.5 rounded-xl bg-gray-50 dark:bg-[#1a1f2e]"
                >
                  <div>
                    <p className="text-sm font-medium text-gray-700 dark:text-gray-200">{c.name}</p>
                    <p className="text-xs text-gray-400">
                      {c.leadCount} prospectos · {c.dealCount} ventas
                    </p>
                  </div>
