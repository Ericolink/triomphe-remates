    ...
    <CollapsibleSection title="Analítica" icon={<BarChart3 size={16} className="text-blue-700 dark:text-blue-400" />}
      ...
      {tab === 'inventario' && (
        <motion.div key="inventario" initial="hidden" animate="visible" variants={staggerContainer}>
          <motion.div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6" variants={staggerContainer}>
          ...
          <motion.div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6" variants={staggerContainer}>
            <motion.div variants={fadeInUp} className="bg-gray-50 dark:bg-[#1a1f2e] rounded-2xl p-5">
              <h3 className="font-semibold text-gray-800 dark:text-gray-100 mb-4 flex items-center gap-2">
                  ...
                  <div key={city}>
                    ...
                    <div className="h-2 bg-gray-100 dark:bg-[#2e3650] rounded-full overflow-hidden">
                      <div className="h-full bg-blue-900 dark:bg-blue-500 rounded-full"
                        style={{ width: `${d.properties.total > 0 ? (total / d.properties.total) * 100 : 0}%` }} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          <motion.div variants={fadeInUp} className="bg-gray-50 dark:bg-[#1a1f2e] rounded-2xl p-5 mb-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-gray-800 dark:text-gray-100 flex items-center gap-2">
                <Building2 size={16} className="text-blue-700 dark:text-blue-400" /> Propiedades recientes
              </h3>
              <button onClick={() => navigate('/admin/propiedades')} className="text-xs font-medium text-blue-600 dark:text-blue-400 hover:underline">
                Ver todas
              </button>
            </div>
          ...
          <motion.div variants={fadeInUp} className="bg-gray-50 dark:bg-[#1a1f2e] rounded-2xl p-5">
            <h3 className="font-semibold text-gray-800 dark:text-gray-100 mb-4">Estatus del inventario</h3>
      ...
      {tab === 'leads' && (
        <motion.div key="leads" initial="hidden" animate="visible" variants={staggerContainer}>
          <motion.div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6" variants={staggerContainer}>
            <motion.div variants={fadeInUp} className="lg:col-span-1 bg-gray-50 dark:bg-[#1a1f2e] rounded-2xl p-5">
              <h3 className="font-semibold text-gray-800 dark:text-gray-100 mb-4 flex items-center gap-2">
              ...
              <div className="space-y-3">
                {d?.leadsByStatus?.map(({ status, total }) => (
                  <div key={status}>
                    ...
                    <div className="h-2 bg-gray-100 dark:bg-[#2e3650] rounded-full overflow-hidden">
                      <div className="h-full bg-blue-900 dark:bg-blue-500 rounded-full"
                        style={{ width: `${d.leads.total > 0 ? (total / d.leads.total) * 100 : 0}%` }} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          <motion.div variants={fadeInUp} className="bg-gray-50 dark:bg-[#1a1f2e] rounded-2xl p-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              {d?.leadsBySource?.filter(({ total }) => total > 0).map(({ source, total }) => (
                <div key={source} className={`rounded-xl p-3 text-center ${SOURCE_COLORS[source] || SOURCE_COLORS.otro}`}>
