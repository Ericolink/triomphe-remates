    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="md:col-span-2">
          <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Nombre de la campaña *</label>
          <input type="text" value={form.name} placeholder="Ej: Remate Polanco Julio"
            onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} className={inputClass} />
        </div>
        <div>
          <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Plataforma</label>
          <select value={form.platform} onChange={(e) => setForm((f) => ({ ...f, platform: e.target.value }))} className={inputClass}>
            {Object.entries(CAMPAIGN_PLATFORM_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
          </select>
        </div>
        <div>
          <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Presupuesto (opcional)</label>
          <input type="number" value={form.budget} placeholder="50000"
            onChange={(e) => setForm((f) => ({ ...f, budget: e.target.value }))} className={inputClass} />
        </div>
        <div>
          <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Fecha de inicio *</label>
          <input type="date" value={form.startDate?.slice(0, 10) || ''}
            onChange={(e) => setForm((f) => ({ ...f, startDate: e.target.value }))} className={inputClass} />
        </div>
        <div>
          <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Fecha de fin (opcional)</label>
      ...
      <div className="flex gap-3 justify-end">
        ...
        <motion.button type="button" onClick={() => onSave(form)}
          disabled={isPending || !form.name || !form.startDate} whileHover={buttonHover} whileTap={buttonTap}
        ...
        {isLoading || !c ? <Spinner size="md" className="py-8" /> : (
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              { label: 'Costo por venta', value: c.metrics.costPerSale ? formatPrice(c.metrics.costPerSale) : '—' },
            ].map(({ label, value }) => (
...
export default function CampanasSection() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Campaña creada'); queryClient.invalidateQueries(['campaigns']); setModal(null); },
    onError: (e) => toast.error(e?.response?.data?.error || 'Error al crear'),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, data: d }) => updateCampaign(id, d),
    onSuccess: () => { toast.success('Campaña actualizada'); queryClient.invalidateQueries(['campaigns']); setModal(null); },
    onError: (e) => toast.error(e?.response?.data?.error || 'Error al actualizar'),
  });

  const deleteMutation = useMutation({
    mutationFn: deleteCampaign,
    onSuccess: () => { toast.success('Campaña eliminada'); queryClient.invalidateQueries(['campaigns']); },
    ...
    <motion.div variants={fadeIn} initial="hidden" animate="visible">
      ...
      {isLoading ? <Spinner size="lg" className="py-16" /> : (
        <motion.div className="space-y-3" variants={staggerContainer} initial="hidden" animate="visible">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <h3 className="font-bold text-gray-800 dark:text-gray-100 truncate">{c.name}</h3>
                  </div>
                ...
                <div className="flex items-center gap-1 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                  ...
                  <OverflowMenu items={[
                    { label: 'Eliminar', icon: <Trash2 size={14} />, danger: true, onClick: () => setConfirm({ title: `¿Eliminar "${c.name}"?`, message: 'Los prospectos asociados no se eliminan, solo quedan sin campaña.', onConfirm: () => { deleteMutation.mutate(c.id); setConfirm(null); } }) },
                  ]} />
                </div>
              </div>
            </motion.div>
          ...
          {campaigns.length === 0 && (
            <div className="text-center py-16 text-gray-400 dark:text-gray-500">No hay campañas. Crea la primera.</div>
          )}
        </motion.div>
      )}
