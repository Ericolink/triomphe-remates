...
export default function AlertSubscriptionForm() {
  ...
  const handleSubmit = (e) => {
    ...
    mutation.mutate({ ...form, maxPrice: form.maxPrice ? form.maxPrice.replace(/[^0-9]/g, '') : undefined });
  };

  const inputCls = 'w-full px-3 py-2 border border-gray-200 dark:border-[#2e3650] rounded-xl text-sm bg-white dark:bg-[#1a1f2e] dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:placeholder-gray-500';

  if (sent) return (
    <motion.div variants={fadeInUp} initial="hidden" animate="visible" className="flex flex-col items-center gap-3 py-6 text-center">
      <CheckCircle size={36} className="text-green-500" />
      <p className="font-semibold text-gray-800 dark:text-gray-100">¡Alerta activada!</p>
      <p className="text-sm text-gray-500 dark:text-gray-400">Te avisaremos cuando llegue una propiedad que coincida.</p>
    </motion.div>
  );
    ...
    <form onSubmit={handleSubmit} className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input name="name" value={form.name} onChange={handleChange} placeholder="Tu nombre" maxLength={100} className={inputCls} />
        </div>
        <div>
      ...
      <div>
        <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Precio máximo (opcional)</label>
        <input name="maxPrice" value={form.maxPrice ? Number(form.maxPrice.replace(/[^0-9]/g,'')).toLocaleString('es-MX') : ''} onChange={handleChange} placeholder="Ej: 1,500,000" className={inputCls} />
      </div>
      <button type="submit" disabled={mutation.isPending}
        ...
        {mutation.isPending ? <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <Bell size={15} />}
