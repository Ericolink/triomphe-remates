...
JWT stored in `localStorage`. The `api.js` axios instance auto-attaches the `Authorization: Bearer` header. On 401, it clears storage and redirects to `/admin/login`. Server middleware chain for protected routes: `authenticate` (validates JWT, attaches `req.user`) → optionally `authorize('admin')` (role check).
...
- **DB schema changes**: Sequelize runs `sync({ alter: false })` in production. New fields/models must be added to the model files; Sequelize will create missing columns on next deploy when temporarily switched to `alter: true`, or via a manual SQL migration.
